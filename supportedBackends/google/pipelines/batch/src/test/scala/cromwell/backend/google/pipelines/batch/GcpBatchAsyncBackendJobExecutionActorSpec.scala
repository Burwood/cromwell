...
class GcpBatchAsyncBackendJobExecutionActorSpec extends TestKitSuite
  ...
  lazy val TestableJesExpressionFunctions: GcpBatchExpressionFunctions = {
    new GcpBatchExpressionFunctions(TestableStandardExpressionFunctionsParams)
  }
  ...
  private def buildInitializationData(jobDescriptor: BackendJobDescriptor, configuration: GcpBatchConfiguration) = {
    ...
    val workflowPaths = GcpBatchWorkflowPaths(
      jobDescriptor.workflowDescriptor, NoCredentials.getInstance(), NoCredentials.getInstance(), configuration, pathBuilders)
    ...
    // requestFactory is one of the parameters I removed from GcpBackendInitializationData
    GcpBackendInitializationData(workflowPaths, runtimeAttributesBuilder, configuration)
  }

  class TestablePipelinesApiJobExecutionActor(params: StandardAsyncExecutionActorParams, functions: GcpBatchExpressionFunctions)
    ...
    def this(jobDescriptor: BackendJobDescriptor,
             ...
             functions: GcpBatchExpressionFunctions = TestableJesExpressionFunctions,
    ...
    override lazy val backendEngineFunctions: GcpBatchExpressionFunctions = functions
  }


  private val runtimeAttributesBuilder = GcpBatchRuntimeAttributes.runtimeAttributesBuilder(batchConfiguration)
  ...
  private def executionActor(jobDescriptor: BackendJobDescriptor,
    ...
    class ExecuteOrRecoverActor extends TestablePipelinesApiJobExecutionActor(jobDescriptor, promise, batchConfiguration, jesSingletonActor = jesSingletonActor, serviceRegistryActor = serviceRegistryActor) {
