...
trait Localization {
  ...
  def localizeActions(createPipelineParameters: CreatePipelineParameters, volumes: List[Volume])
    ...
    val runGcsLocalizationScript = cloudSdkShellAction(
      s"/bin/bash $gcsLocalizationContainerPath")()
    ...
    val localizations = {
      ...
      List (localizeGcsTransferLibrary, localizeGcsLocalizationScript, runGcsLocalizationScript) ++ List (localizeGcsDelocalizationScript)
    }
    ...
    RunnableBuilder.cloudSdkShellAction()
  }
