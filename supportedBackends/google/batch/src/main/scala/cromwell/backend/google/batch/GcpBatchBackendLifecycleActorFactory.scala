...
import cromwell.backend.google.batch.models.{GcpBackendInitializationData, GcpBatchConfiguration, GcpBatchConfigurationAttributes}
  ...
  override def dockerHashCredentials(workflowDescriptor: BackendWorkflowDescriptor, initializationData: Option[BackendInitializationData]): List[Any] = {
    Try(BackendInitializationData.as[GcpBackendInitializationData](initializationData)) match {
      case Success(data) =>
        ...
        val effectiveToken = tokenFromWorkflowOptions.orElse(data.gcpBatchConfiguration.dockerCredentials map {
          _.token
        })
