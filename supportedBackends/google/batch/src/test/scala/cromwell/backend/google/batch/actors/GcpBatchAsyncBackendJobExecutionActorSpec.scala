...
class GcpBatchAsyncBackendJobExecutionActorSpec extends TestKitSuite
  ...
  import GcpBatchApiTestConfig._
  ...
  lazy val TestableJesExpressionFunctions: BatchExpressionFunctions = {
  ...
  class TestableGcpBatchJobExecutionActor(params: StandardAsyncExecutionActorParams, functions: BatchExpressionFunctions)
    ...
    def this(jobDescriptor: BackendJobDescriptor,
             ...
             functions: BatchExpressionFunctions = TestableJesExpressionFunctions,
    ...
    wdlNamespace.toWomExecutable(Option(Inputs.toJson.compactPrint), NoIoFunctionSet, strictValidation = true) match {
      case Right(womExecutable) =>
        ...
        val prefetchedKvEntries = Map(
          GcpBatchBackendLifecycleActorFactory.preemptionCountKey -> KvPair(ScopedKey(workflowDescriptor.id, KvJobKey(key), GcpBatchBackendLifecycleActorFactory.preemptionCountKey), previousPreemptions.toString),
          GcpBatchBackendLifecycleActorFactory.unexpectedRetryCountKey -> KvPair(ScopedKey(workflowDescriptor.id, KvJobKey(key), GcpBatchBackendLifecycleActorFactory.unexpectedRetryCountKey), previousUnexpectedRetries.toString))
  ...
  private def executionActor(jobDescriptor: BackendJobDescriptor,
    ...
    class ExecuteOrRecoverActor extends TestableGcpBatchJobExecutionActor(jobDescriptor, promise, gcpBatchConfiguration, jesSingletonActor = jesSingletonActor, serviceRegistryActor = serviceRegistryActor) {
...
//    val runStatus = UnsuccessfulRunStatus(errorCode, Option(innerErrorMessage), Seq.empty, Option("fakeMachine"), Option("fakeZone"), Option("fakeInstance"), expectPreemptible)
  ...
  def buildPreemptibleTestActorRef(attempt: Int, preemptible: Int, failedRetriesCountOpt: Option[Int] = None): TestActorRef[TestableGcpBatchJobExecutionActor] = {
    ...
    val props = Props(new TestableGcpBatchJobExecutionActor(jobDescriptor, Promise(),
...
//    result.isInstanceOf[FailedRetryableExecutionHandle] shouldBe true
    ...
    val scopedKeyPreempt = ScopedKey(jesBackend.workflowId, KvJobKey(key), GcpBatchBackendLifecycleActorFactory.preemptionCountKey)
    ...
    val scopedKeyUnexpected = ScopedKey(jesBackend.workflowId, KvJobKey(key), GcpBatchBackendLifecycleActorFactory.unexpectedRetryCountKey)
...
//    checkFailedResult(Status.CANCELLED, Option("Operation canceled at")) shouldBe AbortedExecutionHandle
  ...
  TestActorRef[TestableGcpBatchJobExecutionActor] = {
    ...
    womExecutableChecked match {
      case Right(womExecutable) =>
        ...
        val props = Props(new TestableGcpBatchJobExecutionActor(jobDescriptor, Promise(), gcpBatchConfiguration, functions))
        TestActorRef[TestableGcpBatchJobExecutionActor](props, s"TestableJesJobExecutionActor-${jobDescriptor.workflowDescriptor.id}")
      case Left(badness) => fail(badness.toList.mkString(", "))
    }
  }
  ...
  it should "generate correct JesOutputs" in {
    ...
    jesOutputs should contain(GcpBatchFileOutput("out",
  ...
  it should "generate correct JesInputs when a command line contains a write_lines call in it" in {
    ...
    class TestBatchExpressionFunctions extends BatchExpressionFunctions(TestableStandardExpressionFunctionsParams) {
      override def writeFile(path: String, content: String): Future[WomSingleFile] = {
        Future.fromTry(Success(WomSingleFile(s"gs://some/path/file.txt")))
      }
    }

    val functions = new TestBatchExpressionFunctions
  ...
  it should "convert local Paths back to corresponding GCS paths in JesOutputs" in {
    ...
    val jesOutputs = Set(
      ...
      GcpBatchFileOutput("/cromwell_root/path/to/file2", gcsPath("gs://path/to/file2"),
        DefaultPathBuilder.get("/cromwell_root/path/to/file2"), workingDisk, optional = false, secondary = false),
      GcpBatchFileOutput("/cromwell_root/path/to/file3", gcsPath("gs://path/to/file3"),
        DefaultPathBuilder.get("/cromwell_root/path/to/file3"), workingDisk, optional = false, secondary = false),
      GcpBatchFileOutput("/cromwell_root/path/to/file4", gcsPath("gs://path/to/file4"),
        DefaultPathBuilder.get("/cromwell_root/path/to/file4"), workingDisk, optional = false, secondary = false),
      GcpBatchFileOutput("/cromwell_root/path/to/file5", gcsPath("gs://path/to/file5"),
    ...
    def wdlValueToGcsPath(jesOutputs: Set[GcpBatchFileOutput])(womValue: WomValue): WomValue = {
  ...
  it should "create a JesFileInput for the monitoring script, when specified" in {
    ...
    testActorRef.underlyingActor.monitoringScript shouldBe
      Option(GcpBatchFileInput("monitoring-in", gcsPath("gs://path/to/script"), DefaultPathBuilder.get("monitoring.sh"), workingDisk))
  }
  ...
  it should "return JES log paths for non-scattered call" in {
    ...
    val workflowDescriptor = BackendWorkflowDescriptor(
      ...
      WdlNamespaceWithWorkflow.load(
        ...
        Seq.empty[Draft2ImportResolver]).get.workflow.toWomWorkflowDefinition(isASubworkflow = false).getOrElse(fail("failed to get WomDefinition from WdlWorkflow")),
      Map.empty,
      WorkflowOptions.fromJsonString(s""" {"${GcpBatchWorkflowPaths.GcsRootOptionKey}": "gs://path/to/gcs_root"} """).get,
      Labels.empty,
      HogGroup("foo"),
      List.empty,
      None
    ...
    jesBackend.gcpBatchCallPaths.stdout should be(a[GcsPath])
    jesBackend.gcpBatchCallPaths.stdout.pathAsString shouldBe
      "gs://path/to/gcs_root/wf_hello/e6236763-c518-41d0-9688-432549a8bf7c/call-hello/stdout"
    jesBackend.gcpBatchCallPaths.stderr should be(a[GcsPath])
    jesBackend.gcpBatchCallPaths.stderr.pathAsString shouldBe
      "gs://path/to/gcs_root/wf_hello/e6236763-c518-41d0-9688-432549a8bf7c/call-hello/stderr"
    jesBackend.gcpBatchCallPaths.batchLogPath should be(a[GcsPath])
    jesBackend.gcpBatchCallPaths.batchLogPath.pathAsString shouldBe
    ...
    val workflowDescriptor = BackendWorkflowDescriptor(
      ...
      WorkflowOptions.fromJsonString(
        s"""|{
            ...
            |  "${GcpBatchWorkflowPaths.GcsRootOptionKey}": "$jesGcsRoot"
            |}
            |""".stripMargin
      ).get,
      Labels.empty,
...
//        "backendLogs:log" -> s"$jesGcsRoot/wf_hello/$workflowId/call-goodbye/goodbye.log",
        // "callRoot" -> "gs://my-cromwell-workflows-bucket/wf_hello/d648aeb1-7890-4040-94a3-d81f18ce7819/call-goodbye"
...
//        "preemptible" -> "false",
        ...
        // "stderr" -> "gs://my-cromwell-workflows-bucket/wf_hello/d648aeb1-7890-4040-94a3-d81f18ce7819/call-goodbye/stderr"
        "stderr" -> s"$jesGcsRoot/wf_hello/$workflowId/call-goodbye/stderr",
        // "stdout" -> "gs://my-cromwell-workflows-bucket/wf_hello/d648aeb1-7890-4040-94a3-d81f18ce7819/call-goodbye/stdout"
