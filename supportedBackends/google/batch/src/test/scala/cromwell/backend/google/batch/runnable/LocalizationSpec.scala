...
//      Set("commands", "environment", "imageUri", "labels", "mounts")
    ...
    fields.find(_._1.getName == "commands").value.asInstanceOf[(_, java.util.List[_])]._2 should contain theSameElementsAs List(
...
//
    fields.find(_._1.getName == "image_uri").value.asInstanceOf[(_, String)]._2 should be("somerepo/drs-downloader:tagged")
//
...
//    actionLabels.get(tagKey) should be(tagLabel)
  ...
  it should "create the right runnable to localize DRS files using a manifest with requester pays" in {
    ...
    val container = Localization.drsRunnable(manifestPath, Map(tagKey -> tagLabel), Option(requesterPaysProjectId)).getContainer
    val fields = container.getAllFields.asScala
    fields.keySet.map(_.getName) should contain theSameElementsAs Set("commands", "image_uri")
//      Set("commands", "environment", "imageUri", "labels", "mounts")
